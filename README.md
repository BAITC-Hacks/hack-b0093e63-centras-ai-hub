...
**Для жюри:** [сценарий проверки](#8-как-проверить-решение) · [установка и запуск](#7-установка-и-запуск) · [пример переменных окружения](.env.example) · [адрес опубликованного API](#11-развёрнутая-версия).
...
Для демонстрации на ekt.kz используйте расширение:
...
Для подключения без расширения разместите `widget.js` на своём HTTPS-хостинге и добавьте в шаблон страницы:
...
Используйте настоящий API из варианта B или C либо сохранённый API команды. Mock-режим для проверки поиска не подходит.
...
- [`e2e/RESULTS.md`](e2e/RESULTS.md): 6 из 13 проверок, итог `FAIL`, прогон от 23.09.2026. Зафиксированы проблемы отображения ответа, подсветки, заполнения формы и поиска.
